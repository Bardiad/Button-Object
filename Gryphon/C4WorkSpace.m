...
-(void)setup {
    ...
    colorArray = [[NSMutableArray alloc]initWithCapacity:capacity];
    ...
    for(int i = 0 ; i < capacity ; i++){
        ...
        ColorButton *button = [[ColorButton alloc]initWithColor:[colorArray objectAtIndex:i] atX:10+(0.1469f*width)*i atY:10];
        ...
        for (button in buttonArray){
            ...
            //Testing xValue/yValue
            NSLog(@"The xValue is %f and the yValue is %f", button.xValue, button.yValue);
        }
    }
...
{
    UIColor *color5 = [UIColor orangeColor];
    [colorArray replaceObjectAtIndex:3 withObject:color5];
    [self updateColorButtonBG];
}
